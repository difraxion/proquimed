...
<script>
    $('#del_img_btn_" . $id . "').click(function(){
        var delBtn = this;
        $.post('" . base_url() . "admin/scrud/delfile?fileType=img&table=" . $_GET['table'] . "&" . http_build_query($q, '', '&') . "',{src:{file:'" . $attributes['value'] . "',field:'" . $fieldName . "'}},function(){
...
$("#' . $fid . '").change(function(){
    ...
    if (!empty($attributes['value']) && is_file(FCPATH . '/media/files/' . $attributes['value'])) {
        $html .= " <div style='display:inline-block;'>
            <span><a href='".base_url()."admin/download?file=" . $attributes['value'] . "'>" . $attributes['value'] . "</a></span>
...
<script>
    $('#del_file_btn_" . $id . "').click(function(){
        var delBtn = this;
        $.post('" . base_url() . "admin/scrud/delfile?table=" . $_GET['table'] . "&" . http_build_query($q, '', '&') . "',{src:{file:'" . $attributes['value'] . "',field:'" . $fieldName . "'}},function(){
            $(delBtn).parent().remove();
            $('#" . $id . "').val('');
        });
    });
