...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-mini btn-primary" href="'.base_url(). 'admin/language/translate?id={crud_languages.id}">'.$lang->line('translate').'</a>');
		$clsWidth += 60;
...
<script>
            ...
            function crudExport() {
                $('#crudIframe').attr({src: '<?php echo base_url() ?>admin/scrud/exportCsv?table=<?php echo $_GET['table']; ?>&xtype=exportcsv'});
            }
            function crudExportAll() {
                $('#crudIframe').attr({src: '<?php echo base_url() ?>admin/scrud/exportCsv?table=<?php echo $_GET['table']; ?>&xtype=exportcsvall'});
            }
</script>