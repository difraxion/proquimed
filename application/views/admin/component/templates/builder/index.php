...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-mini btn-info" href="'.base_url().'admin/scrud/config?com_id={crud_components.id}" >Build</a>');
		$clsWidth += 130;
                                                ...
                                                if (isset($this->form[$field]) && isset($this->form[$field]['element'][0])) {
                                                    switch (trim(strtolower($this->form[$field]['element'][0]))) {
                                                        ...
                                                        if (file_exists(FCPATH . '/media/files/' . $__value)) {
                                                            echo '<a href="' . base_url() . 'admin/download?file=' . $__value . '">' . $__value . '</a>';
                                                        } else {
                                                            echo $__value;
                                                        }
                                                        break;
                            ...
                            buttons: [
                                {
                                    ...
                                    addClass: 'btn-danger',
                                    click: function(id, data) {
                                    	window.location='<?php echo base_url(); ?>admin/scrud/removeConfig?com_id='+com_id;
                                        $.sModal('close', id);
                                    }
                                },
                                {
...
<script>
            ...
            function crudExport() {
                $('#crudIframe').attr({src: '<?php echo base_url(); ?>admin/scrud/exportCsv.php?table=<?php echo $this->table; ?>&xtype=exportcsv'});
            }
            function crudExportAll() {
                $('#crudIframe').attr({src: '<?php echo base_url(); ?>admin/scrud/exportCsv.php?table=<?php echo $this->table; ?>&xtype=exportcsvall'});
            }
</script>